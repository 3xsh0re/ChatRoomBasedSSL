...
#new
    ...
    def creatlogin(self):
        ...
        # 标签 用户和密码
        self.label_usr = Label(self.fr1, text="用户名：", font=("宋体", 12))
        self.label_usr.grid(row=0, column=0, pady=10)
        self.label_pwd = Label(self.fr1, text="密  码：", font=("宋体", 12))
        self.label_pwd.grid(row=1, column=0)

        # 文本框 用户名
        self.var_usr_name = StringVar()
        self.entry_name = Entry(self.fr1, textvariable=self.var_usr_name, font=("宋体", 12))
        self.entry_name.grid(row=0, column=1)
        self.entry_name.focus_set()  # 获得焦点
        # 文本框 密码
        self.var_usr_pwd = StringVar()
        self.entry_pwd = Entry(self.fr1, textvariable=self.var_usr_pwd, show="*", font=("宋体", 12))
        ...
        self.bt_register = Button(self.fr4, text=" 账号注册", relief=FLAT, bg='#f0f0f0', command=self.login_win_close)
